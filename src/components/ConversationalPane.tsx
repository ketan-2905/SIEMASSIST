...
//   const handleRunQuery = async (messageId: string) => {
//     console.log(messageId + " message From handleRunQuery");
//     const message = messages.find((m) => m.id === messageId);
//     console.log(message + " message From handleRunQuery");
...
//     // Remove these lines since we're already setting status to "running" before calling this function
//     // const runningMsgs = messages.map((m) =>
//     //   m.id === messageId ? { ...m, status: "running" as MessageStatus } : m
//     // );
//     // setMessages(runningMsgs);
...
//       // updateConversationMessages(withReady);
//       // // setTimeout(() => handleRunQuery(readyMsg.id), 2000);
//       // // Immediately set to running state and run the query
//       // const runningMsg = { ...readyMsg, status: "running" as MessageStatus };
//       // const withRunning = withReady.map((m) =>
//       //   m.id === assistantMsgId ? runningMsg : m
//       // );
//       // setMessages(withRunning);
//       // updateConversationMessages(withRunning);

//       // Run the query immediately
//       // handleRunQuery(readyMsg.id);

//       const parentMessage = readyMsg.parentId
//         ? messages.find((m) => m.id === readyMsg.parentId)
//         : undefined;
//       const queryType = determineQueryType(readyMsg.query as string);

//       try {
//         const results = await runQuerySimulator(
//           readyMsg.query as string,
//           queryType,
//           parentMessage
//         );
//         const doneMsgs = messages.map((m) =>
...
//       } catch (e) {
//         const errorMsgs = messages.map((m) =>
//           m.id === readyMsg.id ? { ...m, status: "error" as MessageStatus } : m
//         );
//         setMessages(errorMsgs);
//         updateConversationMessages(errorMsgs);
//       }
...
//                       <div className="flex space-x-2 mt-3">
//                         {message.status === "running" && (
//                           <div className="flex items-center space-x-1 px-3 py-1.5 bg-indigo-600 text-white text-xs rounded">
...
const ConversationalPaneWithSidebar: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const runningMsg: Message = {
        ...generatingMsg,
        content: `I'll help you with "${userInput}". Running Elasticsearch DSL query...`,
        query,
        queryLanguage,
        status: "running",
        queryId: qid,
      ...
      const withRunning = withGenerating.map((m) =>
        m.id === assistantMsgId ? runningMsg : m
      );
      setMessages(withRunning);
      updateConversationMessages(withRunning);

      // Now run the query with the complete message data
      const parentMessage = runningMsg.parentId
        ? messages.find((m) => m.id === runningMsg.parentId)
        : undefined;

      const results = await runQuerySimulator(
        query,
        queryType,
        parentMessage?.results
      );
      ...
      const withDone = withRunning.map((m) =>
        m.id === assistantMsgId ? doneMsg : m
      );
      setMessages(withDone);
      updateConversationMessages(withDone);
    } catch (e) {
      const errorMsg = withGenerating.map((m) =>
        m.id === assistantMsgId
          ? {
              ...m,
              status: "error" as MessageStatus,
              content: "Failed to generate or run query",
            }
          : m
      );
      setMessages(errorMsg);
  ...
  const handleManualRunQuery = async (messageId: string) => {
    const message = messages.find((m) => m.id === messageId);
    if (!message?.query) return;
  ...
  const renderAnalysis = (message: Message) => {
    ...
    return (
      <div className="mt-4 pt-4 border-t border-gray-700">
        <button className="flex items-center space-x-2 text-sm font-semibold text-emerald-400 hover:text-emerald-300 mb-2">
          <BarChart3 size={16} />
          <span>Security Analysis</span>
        </button>
        ...
        <div className="flex-none px-6 py-4 border-b border-gray-800 bg-gradient-to-r from-emerald-900/10 to-blue-900/10">
          <div className="flex items-center justify-between">
            ...
            <div className="flex justify-between items-center gap-2">
              ...
              <div className="text-xs text-gray-500 bg-gray-800 h-9 px-4 rounded flex items-center border border-gray-700 hover:border-gray-500 transition-colors cursor-pointer">
                <Home size={16} className="inline-block mr-1" />
              </div>
            </div>
          </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-700">
                      ...
                      <div className="flex space-x-2 mt-3">
                        {(message.status === "ready" ||
                          ...
                          <button
                            onClick={() => handleManualRunQuery(message.id)}
                            className="flex items-center space-x-1 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs rounded transition-colors"
                          >
                         ...
                         {message.status === "running" && (
                          <div className="flex items-center space-x-2 px-3 py-1.5 bg-slate-600 text-white text-xs rounded transition-colors">Running..</div>
                         )}
                          ...
                          message.status !== "generating" && (
                            <>
                              <button
                                ...
                                className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-gray-300 text-xs rounded transition-colors"
                              >
                                {expandedQuery === message.id ? "Hide" : "View"}{" "}
                                Query
                              </button>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-700">
                      <div className="bg-[#08090b] p-4 rounded border border-emerald-900/30 shadow-lg">
                        ...
                        <div className="mb-3 flex items-center space-x-4 justify-between">
                          ...
                          <button
                            onClick={() => toggleAnalysis(message.id)}
                            className="flex items-center space-x-2 text-sm font-semibold text-emerald-400 hover:text-emerald-300"
                          >
                            <span>Query Result</span>
                            <ChevronDown
                              size={16}
