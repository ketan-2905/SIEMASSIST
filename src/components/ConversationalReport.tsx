...
//           {/* Explanatory text for User Login Chart */}
//           <div className="mt-4 p-3 bg-gray-800/30 rounded border border-gray-700">
//             <h4 className="text-xs font-semibold text-gray-200 mb-2">User Activity Analysis</h4>
//             <p className="text-xs text-gray-400">
//               This chart shows the distribution of VPN login attempts across different users. 
//               User <strong className="text-emerald-400">john</strong> shows the highest activity with 4 login attempts, 
//               indicating either regular usage patterns or potential security review requirements. 
//               The distribution helps identify both frequent users and potential anomalies in access patterns.
//             </p>
//           </div>
//         </div>
...
//           {/* Explanatory text for Success/Failure Chart */}
//           <div className="mt-4 p-3 bg-gray-800/30 rounded border border-gray-700">
//             <h4 className="text-xs font-semibold text-gray-200 mb-2">Authentication Health Check</h4>
//             <p className="text-xs text-gray-400">
//               The authentication success rate of <strong className="text-green-400">61.5%</strong> (8 successful vs 5 failed logins) 
//               indicates potential security concerns. The 38.5% failure rate suggests possible credential issues, 
//               brute-force attempts, or authentication configuration problems that require immediate investigation 
//               and monitoring.
//             </p>
//           </div>
//         </div>
...
//       {/* Login attempts in last 10 hours - Line Chart */}
//       <div className="bg-[#08090b] p-4 rounded border border-emerald-900/30 shadow-lg">
//         <div className="flex items-center gap-2 mb-4">
//           <BarChart3 size={16} className="text-emerald-400" />
//           <h3 className="text-sm font-semibold text-gray-200">
...
//               {/* Explanatory text for Time-based Activity Chart */}
//               <div className="mt-4 p-3 bg-gray-800/30 rounded border border-gray-700">
//                 <h4 className="text-xs font-semibold text-gray-200 mb-2">Temporal Activity Pattern</h4>
//                 <p className="text-xs text-gray-400">
//                   This timeline shows VPN login patterns over the last 10 hours. The peak activity around 06:00 UTC 
//                   (8 attempts) indicates potential coordinated access or scheduled automated connections. 
//                   The drop to zero attempts at 08:00 UTC followed by a resurgence suggests normal business hour patterns 
//                   with potential off-hours activity that may require security review.
//                 </p>
//               </div>
//             </>
//           );
...
//               <li>• User 'john' shows highest activity with 4 login attempts from multiple IPs</li>
//               <li>• 38.5% authentication failure rate indicates potential security risks</li>
//               <li>• Root account accessed from non-standard workstations (WORKSTATION-01)</li>
//               <li>• Multiple failed attempts from external IPs 198.51.100.33 and 14.192.32.101</li>
//             </ul>
//           </div>
//           <div>
//             <h4 className="text-gray-200 font-medium mb-2">Recommended Actions</h4>
//             <ul className="space-y-1 text-xs">
//               <li>• Investigate repeated failed login patterns for potential brute-force attacks</li>
//               <li>• Review root account access from non-server systems for compliance</li>
//               <li>• Monitor IP addresses 14.192.32.101 and 198.51.100.33 for suspicious activity</li>
//               <li>• Consider implementing additional authentication factors for sensitive accounts</li>
//             </ul>
//           </div>
//         </div>
//       </div>
...
const ReportCharts = () => {
  // Build userLoginData
  const userLoginData = vpnData.reduce((acc: UserLoginData[], d) => {
    const existing = acc.find((u) => u.name === d.username);
    if (existing) {
      existing.logins += 1;
    } else {
  ...
  // Host activity data
  const hostActivityData = vpnData.reduce((acc: HostActivityData[], d) => {
    const existing = acc.find((h) => h.name === d.host);
    if (existing) {
      existing.attempts += 1;
    } else {
    ...
    try {
      ...
      await Promise.all(
        Array.from(plotlyCanvases).map(
          (plot) =>
            new Promise((resolve) => {
              const plotElement = plot as HTMLElement & { _fullLayout?: unknown };
              if (plotElement._fullLayout) resolve(true);
              else setTimeout(resolve, 300);
            })
        )
      );
