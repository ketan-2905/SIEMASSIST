...
const ConversationalPaneWithSidebar: React.FC = () => {
    ...
    const generateQueryFromNL = async (nl: string, parentMessage?: Message | null): Promise<{ query: string; queryLanguage: QueryLanguage; analysis?: string }> => {
        ...
        const parentIps = parentEntities.filter(e => e.type === 'ip').map(e => e.value);
        const parentUsers = parentEntities.filter(e => e.type === 'user').map(e => e.value);
        const parentHosts = parentEntities.filter(e => e.type === 'host').map(e => e.value);
        ...
        if (/vpn/i.test(nl) && !parentMessage) {
            ...
            analysis = "This query searches for all VPN connection attempts in the last 24 hours. It will help identify suspicious VPN activities and potential unauthorized access attempts.";

        }
        // Query 2: Chain from VPN to find failed attempts
        else if ((/failed logins?|failed auth|failure/i.test(nl) && parentMessage) ||
            ...
            analysis = "Chained from previous VPN query - now filtering only FAILED login attempts. This helps identify potential brute-force attacks or unauthorized access attempts from specific IP addresses.";

        }
        // Query 3: Chain to find successful logins from suspicious IPs
        else if ((/successful|success.*login/i.test(nl) && parentMessage) ||
            (/check.*success/i.test(nl) && parentMessage)) {

            const ips = parentIps.length > 0 ? parentIps.slice(0, 3) : ["192.168.1.10", "192.168.1.15", "203.0.113.2"];
            ...
            analysis = `Chained query checking if previously identified suspicious IPs [${ips.join(", ")}] had successful SSH logins. This detects potential brute-force attack successes.`;

        }
        // Query 4: Malware detection on compromised hosts
        else if ((/malware|virus|threat/i.test(nl) && parentMessage) ||
            (/check.*malware/i.test(nl) && parentMessage)) {

            const hosts = parentHosts.length > 0 ? parentHosts.slice(0, 3) : ["SERVER-02", "WEB-03", "DB-01"];
            ...
            baseQuery = JSON.stringify({
                ...
                _source: ["@timestamp", "host.name", "malware.name", "severity", "event.action"],
                size: 100
            }, null, 2);

            analysis = `Chained malware detection query on hosts [${hosts.join(", ")}] that showed suspicious login activity. This helps identify if compromised credentials led to malware infections.`;
        ...
        // Default security investigation query
            ...
            baseQuery = JSON.stringify({
                ...
                _source: ["@timestamp", "source.ip", "user.name", "event.outcome", "host.name"],
                size: 100
            }, null, 2);

            analysis = "General security investigation query covering authentication and network events from the last 24 hours. Use this as a starting point for security analysis.";
        }
    ...
    const generateMockLogs = (queryType: string, parentResults?: any) => {
        const users = ["john", "admin", "neha", "raj", "alice", "bob", "diana", "root", "svc_account"];
        const ips = ["192.168.1.45", "203.0.113.22", "14.192.32.101", "172.16.2.44", "198.51.100.33", "10.1.1.100"];
        const hosts = ["SERVER-01", "SERVER-02", "WEB-01", "DB-01", "WORKSTATION-01"];
        const malwareNames = ["Trojan.Generic", "Backdoor.Agent", "Ransomware.Crypto", "Spyware.Keylogger"];
        ...
        for (let i = 0; i < numLogs; i++) {
            ...
            if (queryType === "vpn" || queryType === "auth") {
                ...
                logs.push({
                    ...
                    "malware.name": malwareNames[Math.floor(Math.random() * malwareNames.length)],
                    "severity": ["LOW", "MEDIUM", "HIGH", "CRITICAL"][Math.floor(Math.random() * 4)],
                    "host.name": targetHosts[Math.floor(Math.random() * targetHosts.length)],
    ...
    const runQuerySimulator = async (query: string, queryType: string, parentMessage?: Message) => {
        ...
        rows.forEach((r) => {
            ...
            if (r["user.name"]) userCounts[r["user.name"]] = (userCounts[r["user.name"]] || 0) + 1;
        ...
        return {
            ...
            analysis: {
                ...
                failure_rate: total_hits > 0 ? ((failed / total_hits) * 100).toFixed(1) : "0",
    ...
    const handleSend = async () => {
        ...
        const isChained = lastAssistantMsg && (/filter|narrow|only|check|success|malware|chain/i.test(userInput) ||
    ...
    const handleRunQuery = async (messageId: string) => {
        ...
        const parentMessage = message.parentId ? messages.find(m => m.id === message.parentId) : undefined;
        ...
        return (
            <div className="mt-4 pt-4 border-t border-gray-700">
                ...
                >
                    ...
                    <ChevronDown
                        size={16}
                        className={`transform transition-transform ${showAnalysis[message.id] ? 'rotate-180' : ''}`}
                    />
                </button>
                ...
                {showAnalysis[message.id] && (
                    <div className="bg-gray-900/50 rounded-lg p-4 border border-gray-700">
                        {message.analysis && (
                            <div className="mb-4">
                                <h4 className="text-sm font-semibold text-gray-300 mb-2">Query Purpose:</h4>
                                <p className="text-sm text-gray-400 leading-relaxed">{message.analysis}</p>
                            </div>
                        )}

                        {message.results?.analysis && (
                            <div>
                                <h4 className="text-sm font-semibold text-gray-300 mb-2">Results Analysis:</h4>
                                <div className="grid grid-cols-2 gap-4 text-xs">
                                    <div className="bg-gray-800/50 p-3 rounded">
                                        <div className="text-gray-400">Total Events</div>
                                        <div className="text-white font-mono">{message.results.analysis.total_events}</div>
                                    </div>
                                    <div className="bg-gray-800/50 p-3 rounded">
                                        <div className="text-gray-400">Failure Rate</div>
                                        <div className="text-red-400 font-mono">{message.results.analysis.failure_rate}%</div>
                                    </div>
                ...
                <div className="flex-none px-6 py-4 border-b border-gray-800 bg-gradient-to-r from-emerald-900/10 to-blue-900/10">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-xs text-gray-500 bg-gray-800 px-3 py-1 rounded">
                            <Link href="/" ><Home size={16} className="inline-block mr-1" /></Link>
                        </div>
                    </div>
                </div>
                    ...
                    {messages.length === 0 && (
                        <div className="flex items-center justify-center h-full">
                            <div className="text-center max-w-md">
                                ...
                                <h3 className="text-xl font-semibold mb-2">Start Your Security Investigation</h3>
                                <p className="text-gray-400 text-sm mb-4">
                                    Ask me to search for security events, analyze logs, or generate Elasticsearch DSL queries with chaining.
                                </p>
                                    ...
                                    {message.query && (
                                        <div className="mt-4 pt-4 border-t border-gray-700">
                                            ...
                                            <div className="flex space-x-2 mt-3">
                                                ...
                                                >
                                                    <span>{message.status === "running" ? "Running..." : "Edit query"}</span>
                                                </button>
